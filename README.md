...
The desktop shell follows one contextual workspace model: the upper-left switch selects Orbit, Training, or Orbit Code. Each sidebar then shows one create action, that workspace's fixed destinations, and only that workspace's history. Orbit shows Orbit conversations; Training shows models, collaboration, and training history; Orbit Code shows plugins, API/model configuration, and Code conversations. Collaboration belongs only to Training. The translucent sidebar and the liquid-glass sidebar/back/forward control group follow the same compact hierarchy as a modern coding workspace. Both Orbit chat and Orbit Code use one continuously scrollable conversation surface with the composer pinned at the bottom; the former no longer duplicates a runtime inspector or page header. Execution plans, live actions, elapsed time, file deltas, tool output, and the final summary stay inside the conversation instead of occupying a separate dashboard above it. Collapsed execution stages first report the tool-call count (for example, “Used 2 tools”), followed by file, command, and search counts; expansion reveals every tool and its full output. The model is shown in the lower-right composer; opening it reveals the Intelligence slider, while Advanced contains the exact model and speed controls.
...
- Model connections include local Orbit checkpoints, OpenAI, Anthropic, Gemini, DeepSeek, xAI, Groq, Mistral, OpenRouter, and custom OpenAI-compatible endpoints. Orbit Code always lists verified local checkpoints alongside saved API profiles, and keeps its workspace path in Advanced. Multiple API profiles are stored locally with hidden keys. Providers that reject optional OpenAI fields receive one portable-core retry.
