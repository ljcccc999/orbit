...
- Permission choices are **Ask for approval**, **Approve workspace actions**, and **Full access**. Destructive or out-of-scope actions still pass through the local permission flow. Computer control is a separate, default-off setting: the agent prefers files, APIs, and command-line tools and only drives the mouse or keyboard when no reliable programmatic route is available. Every attempted computer action appears in the execution timeline.
- Closing or quitting the desktop UI leaves the Agent and local API running. **Stop Background Agent** is the separate explicit shutdown action. Settings can prevent sleep while Orbit works, enable or disable login-time background service startup, and allow computer control.
