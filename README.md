...
- The four optimization choices also recommend AI-assisted sample counts and manual corpus size together. AI assistance accepts 1–50,000 samples; a manually entered count is never silently replaced by a recommendation. Manual text and AI-generated text can be merged in one run, while completed or partially generated teacher content remains separately viewable after an interruption. See `docs/300M-38B-全模型规模参数推荐.md`.
...
For scale: a 300M model trained from random weights is a foundation-model project, not a normal local chat fine-tune. The Chinchilla 20-token-per-parameter reference is about **6 billion tokens** for 300M parameters—roughly 888,889 documents at Orbit's current measured 6,750-byte average, or about 732,422 optimizer updates at sequence 1,024, batch 1 and accumulation 8. This is a planning reference, not a guarantee of quality. The practical chat route is a genuinely pretrained compatible base followed by high-quality SFT: 50,000 samples is a minimum starting range, 100,000 is the default recommendation, and 200,000 is an effect-first range. Because one AI generation run is capped at 50,000 samples, 100,000 means two reviewed 50,000-sample rounds. At 100,000 samples averaging 512 tokens, sequence 1,024, batch 1, accumulation 8 and two corpus passes correspond to about 12,500 optimizer updates.
