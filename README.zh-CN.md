...
- 四种参数优化目标会同时推荐 AI 辅助样本数和人工文献字数；AI 辅助支持 1～5,000 个样本，人工文献和 AI 语料可以合并在同一次训练中，教师原文仍可单独查看。完整表见 `docs/300M-38B-全模型规模参数推荐.md`。

页面会分开显示人工训练建议：预训练第 1～N 次都应以文献和代码为主，只在末尾加入少量对话/身份样本；微调应使用专业文献、单轮标注任务和约 50% 高质量指令对话。结构化任务包括分类/情感分析、NER、代码/SQL 生成、摘要总结、文本扩写/润色；它们是输入→输出样本，不是聊天气泡。外部或下载模型不能从随机权重开始训练，只能作为基础模型进入微调。
