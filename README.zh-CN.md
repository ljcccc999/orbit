...
3. **生成样本并自动训练**：调用 DeepSeek 或其他 OpenAI 兼容教师 API。Orbit 会把所选模型的参数量、上下文长度、步数、父模型、用户目标和不可变身份规则告诉教师模型：模型是由 YUNSH 开发的 Orbit。每个自动生成的数据集还会在训练前加入真实的身份问答样本。样本数会影响数据覆盖面、生成时间和 API 费用。生成的数据集保存在本机。每个提供商的模型、地址和 Key 分别保存在 `~/.orbit/teacher-api.json`，权限仅限当前用户；切换回来会恢复原配置，输入新 Key 只替换当前提供商的旧 Key。Key 不会进入训练历史或导出包。训练目标会发送给提供商且可能产生费用，因此必须由用户明确确认。
