...
- 四种参数优化目标会根据实际 token 预算、模型规模、设备和内存推荐参数；AI 辅助每轮支持 1～10,000,000 个样本，这是防止误触造成 API、磁盘和内存失控的运行上限，不是完整预训练语料。用户手动填写的数量不会被推荐值悄悄覆盖；更大的语料应分轮生成、查看、去重和审核。人工文献和 AI 语料可以合并在同一次训练中，已完成或中断前的教师原文仍可单独查看。完整表见 `docs/300M-38B-全模型规模参数推荐.md`。
