...
- 四种参数优化目标会同时推荐 AI 辅助样本数和人工文献字数；AI 辅助支持 1～50,000 个样本，用户手动填写的数量不会被推荐值悄悄覆盖。人工文献和 AI 语料可以合并在同一次训练中；即使生成中断，已经完成的教师原文也会保留并可单独查看。完整表见 `docs/300M-38B-全模型规模参数推荐.md`。
...
规模上，300M 随机权重从零预训练是基础模型工程，不是普通本机聊天微调。按 Chinchilla 的每参数约 20 token 参考值，300M 约需 **60 亿 token**；按 Orbit 当前实测每篇约 6,750 字节估算，约需 888,889 篇，或在序列 1,024、Batch 1、梯度累计 8 时约 732,422 次优化器更新。这只是规划参考，不保证质量。真正可行的聊天路线是选择已经完成基础预训练且格式兼容的基础模型，再做高质量 SFT：5 万条是最低起点，10 万条是默认建议，20 万条是效果优先范围。单次 AI 生成上限为 5 万条，因此 10 万条应分两次各 5 万条生成、审核和训练。若 10 万条平均每条 512 token，序列 1,024、Batch 1、梯度累计 8、训练 2 个数据遍历约等于 12,500 次优化器更新。
