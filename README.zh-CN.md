...
- **预训练（从零开始）**使用随机初始化的 Orbit 权重；**微调**必须选择兼容的本机 Orbit checkpoint，并会保留父模型训练血缘。Orbit 不会把用户选择的模式静默改成另一种。
...
300M–38B 表示 Orbit 的架构规模，不是已经预训练好的模型下载。当前 Orbit 的 byte-level 架构和训练器还不能直接加载 Hugging Face/Qwen 权重；要把 Qwen 作为微调基础模型，必须先完成真实的 tokenizer、模型架构和权重格式适配。要从零训练出真正可用的基础模型，需要大量经过认真处理的数据和可观的算力。
